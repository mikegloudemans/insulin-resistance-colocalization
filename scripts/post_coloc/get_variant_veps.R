...
max_num_loci = 100000
all_loci = as.data.frame(list(chr=rep(0, max_num_loci), pos=rep(0, max_num_loci), locus=rep(0, max_num_loci), vep = rep("", max_num_loci), ref=rep("", max_num_loci), alt=rep("", max_num_loci), gene=rep("", max_num_loci), feature=rep("", max_num_loci), r2=rep("", max_num_loci)), stringsAsFactors=FALSE)
loc_table_idx = 0
# For each locus
for (l in 1:max(colocs$locus))
{
	print(l)
	# Get all lead SNPs for that locus
	...
	{	
		...
		# NOTE: This isn't perfect but it's fine, will do the job for now
		# TODO: Get all SNPs that are LD buddies of any of those lead SNPs (>0.8) and add them to a "set" for the locus
		ld = unlist(system(sprintf("tabix %s chr%s:%s-%s", ld_file, as.character(locus_snps$chr[snp]), as.character(locus_snps$pos[snp]), as.character(locus_snps$pos[snp])), intern=TRUE))
		if (length(ld)==0)
		{
			next
		}
	...
	{
		out = unlist(system(sprintf("tabix %s %s:%s-%s", cadd_file, as.character(all_snps$chr[snp]), as.character(all_snps$pos[snp]), as.character(all_snps$pos[snp])), intern=TRUE))
