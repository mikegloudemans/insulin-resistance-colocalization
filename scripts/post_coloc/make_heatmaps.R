...
get_file = function(coloc_file)
{
	# Read coloc results
	coloc_res=fread(file = coloc_file, sep = '\t', header = T, check.names = F)

	# TODO: This should really be done in the file that gets the HGNC stuff in the first place
	...
	# NOTE: This might have to be changed if more QTL types being used
	# Should be specified more explicitly in config file which is which
	coloc_res$qtl_type = ifelse(grepl("eQTL", coloc_res$eqtl_file), "eqtl", "sqtl")

	# TODO: should be dealt with in config file
	...
	# TODO: IF annotating with crosses and other marks to denote which ones 
...
# ----------
# TODO: This should be handled from within the 
...
# TODO: This collapsing should be done in a way that's specified entirely within the config file
# And is specific to each plot being generated (but could be done multiple times)
...
# TODO: Deduplication should occur in same function as above
# and should be done after filtering correctly
