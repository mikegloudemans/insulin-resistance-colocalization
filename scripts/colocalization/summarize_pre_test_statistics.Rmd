...
```{r include=FALSE}
kept_traits = c("/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/BMI_GIANT_2018/BMI_GIANT_2018.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/FastGlu_MAGIC_Europeans/FastGlu_MAGIC_Europeans.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/FastInsu_adjBMI_MAGIC_Europeans/FastInsu_adjBMI_MAGIC_Europeans.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/HDL_GLGC_Expanded/HDL_GLGC_Expanded.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/TG_GLGC_Expanded/TG_GLGC_Expanded.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/MI_adjBMI/MI_adjBMI.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/MAGIC_ISI_Model_2_AgeSexBMI.txt/MAGIC_ISI_Model_2_AgeSexBMI.txt.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/T2D_Mahajan_Europeans/T2D_Mahajan_Europeans.txt.gz",
		"/users/mgloud/projects/insulin_resistance/data/gwas/formatted/sumstats/WHR-adj-BMI_GIANT_2018/WHR-adj-BMI_GIANT_2018.txt.gz")
kept_eqtls = c("/users/mgloud/projects/brain_gwas/data/eqtls/gtex_v8/Liver.allpairs.txt.gz.eQTLs.txt.gz",
	       "/users/mgloud/projects/brain_gwas/data/eqtls/gtex_v8/Adipose_Visceral_Omentum.allpairs.txt.gz.eQTLs.txt.gz",
	       "/users/mgloud/projects/brain_gwas/data/eqtls/gtex_v8/Adipose_Subcutaneous.allpairs.txt.gz.eQTLs.txt.gz",
	       "/users/mgloud/projects/brain_gwas/data/eqtls/gtex_v8/Muscle_Skeletal.allpairs.txt.gz.eQTLs.txt.gz")
gwas_snp_list = "/users/mgloud/projects/insulin_resistance/output/test_snps/ir-v8_all-gwas_gwas-pval1e-05_gwas-window500000_snps-considered.txt"
coloc_out_dir = "/users/mgloud/projects/brain_gwas/output/ir-v8/"
...
```{r include=FALSE}
kept_gwas_sub = sapply(kept_traits, function(x)
	{
		all = strsplit(x, "/")[[1]]
		trait = gsub("\\.", "_", all[length(all)])
	})
kept_eqtl_sub = sapply(kept_eqtls, function(x)
	{
		all = strsplit(x, "/")[[1]]
		trait = gsub("\\.", "_", all[length(all)])
	})
...
```{r}
all_gwas_snps = read.table(gwas_snp_list, col.names=c("chr", "snp.pos", "pvalue", "trait", "gwas_file"), fill=TRUE, skip=1)
all_gwas_snps = all_gwas_snps[all_gwas_snps$gwas_file %in% kept_traits,]
all_gwas_snps = all_gwas_snps[(all_gwas_snps$pvalue <= 5e-8) | (all_gwas_snps$pvalue <= 1e-5 & (grepl("ISI", all_gwas_snps$trait) | (grepl("MI_adj", all_gwas_snps$trait)))),]
...
```{r}
all_snp_gene_pairs = read.table("/users/mgloud/projects/insulin_resistance/output/test_snps/ir-v8_all-gwas_gtex-ir_gwas-pval1e-05_eqtl-pval1e-05_gwas-window500000_eqtl-window0_snp-gene-pairs-considered.txt", col.names=c("chr", "snp.pos", "gwas_pvalue", "trait", "eqtl_pvalue", "gwas_file", "feature", "eqtl_file"), skip=1)
all_snp_gene_pairs = all_snp_gene_pairs[all_snp_gene_pairs$gwas_file %in% kept_traits,]
all_snp_gene_pairs = all_snp_gene_pairs[all_snp_gene_pairs$eqtl_file %in% kept_eqtls,]
all_snp_gene_pairs$ref_snp = paste(all_snp_gene_pairs$chr, all_snp_gene_pairs$snp.pos, sep="_")
all_snp_gene_pairs = all_snp_gene_pairs[all_snp_gene_pairs$ref_snp %in% unique(all_gwas_snps$ref_snp),]	# Because some might not pass the latest cutoffs
...
```{r}
testable_snps = read.table("/users/mgloud/projects/insulin_resistance/output/test_snps/ir-v8_all-gwas_gtex-ir_gwas-pval1e-05_eqtl-pval1e-05_gwas-window500000_eqtl-window0_coloc-tests.txt", header=TRUE)
testable_snps = testable_snps[testable_snps$gwas_file %in% kept_traits,]
testable_snps = testable_snps[testable_snps$eqtl_file %in% kept_eqtls,]
testable_snps$ref_snp = paste(testable_snps$chr, testable_snps$snp_pos, sep="_")
testable_snps = testable_snps[testable_snps$ref_snp %in% unique(all_gwas_snps$ref_snp),]	# Because some might not pass the latest cutoffs
...
{
	files = dir(paste0(coloc_out_dir, folder))
	...
	if (length(error) > 0)
	{
		err_tab[[i]] = read.table(paste(coloc_out_dir, folder, error, sep="/"), header=FALSE, sep="\t", fill=TRUE, col.names = c("gwas_file", "eqtl_file", "snp.chrom", "snp.pos", "restrict_gene", "trait", "error", "error2"))
		err_tab[[i]]$error = paste(err_tab[[i]]$error, err_tab[[i]]$error2, sep="|")
	}
	if (length(skip) > 0)
	{
		skip_tab[[i]] = read.table(paste(coloc_out_dir, folder, skip, sep="/"), header=FALSE, sep="\t", fill=TRUE, col.names = c("gwas_file", "eqtl_file", "snp.chrom", "snp.pos", "feature", "error", "gwas_data"))
	}

	for (file in files)
	{
		tabs[[i]] = read.table(paste(coloc_out_dir, folder, file, sep="/"), header=TRUE)
		i = i + 1
	}
}
...
results = results[results$base_gwas_file %in% kept_gwas_sub,]
results = results[results$eqtl_file %in% kept_eqtl_sub,]
errors = errors[errors$gwas_file %in% kept_traits,]
errors = errors[errors$eqtl_file %in% kept_eqtls,]
skips = skips[skips$gwas_file %in% kept_traits,]
skips = skips[skips$eqtl_file %in% kept_eqtls,]
...
results$eqtl_file = gsub("_txt_gz", "", results$eqtl_file)
...
{
	if (insignificant_gwas$best_gwas_pval[i] < 5)
	{
		return(FALSE)
	}
	ref = insignificant_gwas$ref_snp[i]
	feature = insignificant_gwas$feature[i]
	# TODO: This is really buggy allowing exceptions like this. FIX IT
	return(sum(grepl("_ISI_|/MI", all_gwas_snps[all_gwas_snps$ref_snp == ref,]$trait)) > 0)
})
...
write.table(missing_pairs[c("ref_snp", "feature", "reason_missing")], file="/users/mgloud/projects/insulin_resistance/output/all_missing_pairs_explained.txt", quote=FALSE, sep="\t", col.names=TRUE, row.names=FALSE)
write.table(results, file="/users/mgloud/projects/insulin_resistance/output/full_coloc_results_qced.txt", quote=FALSE, sep="\t", col.names=TRUE, row.names=FALSE)
```
