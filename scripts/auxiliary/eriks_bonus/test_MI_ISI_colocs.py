...
def main():
    ...
    subprocess.call("rm -rf /users/mgloud/projects/brain_gwas/output/loose-mi-isi-colocs/*", shell=True)
    subprocess.call("rm -rf /users/mgloud/projects/insulin_resistance/tmp/*", shell=True)

    kept_data = set([])
    with open("/users/mgloud/projects/insulin_resistance/eriks_bonus/output/mi-isi_gwas-t2d-tg-ldl_ir-tissues_gwas-pval1e-05_eqtl-pval1e-05_gwas-window1000000_eqtl-window1_coloc-tests.txt") as f:
        all_data = []
        f.readline()
        for line in f:
...
template = '''
{
        "out_dir_group": "loose-mi-isi-colocs",
