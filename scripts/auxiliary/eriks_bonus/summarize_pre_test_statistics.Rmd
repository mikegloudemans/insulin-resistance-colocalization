...
```{r include=FALSE}
kept_traits = c("/users/mgloud/projects/insulin_resistance/eriks_bonus/data/formatted/GWAS_HDL_GLGC_EuropeansMaybe.txt.gz",
	        "/users/mgloud/projects/insulin_resistance/eriks_bonus/data/formatted/GWAS_T2D_Mahajan_Europeans.txt.gz",
            	"/users/mgloud/projects/insulin_resistance/eriks_bonus/data/formatted/GWAS_TG_GLGC_EuropeansMaybe.txt.gz")
...
```{r}
all_gwas_snps = read.table("/users/mgloud/projects/insulin_resistance/eriks_bonus/output/t2d-tg-ldl_gwas-t2d-tg-ldl_gwas-pval1e-05_gwas-window1000000_snps-considered.txt", col.names=c("chr", "snp.pos", "pvalue", "trait", "gwas_file"), skip=1, header=FALSE)
...
write.table(all_gwas_snps, file="/users/mgloud/projects/insulin_resistance/eriks_bonus/output/all_gwas_snps.txt", quote=FALSE, col.names=TRUE, row.names=FALSE, sep="\t")
```
...
```{r}
all_snp_gene_pairs = read.table("/users/mgloud/projects/insulin_resistance/eriks_bonus/output/t2d-tg-ldl_gwas-t2d-tg-ldl_ir-tissues_gwas-pval1e-05_eqtl-pval1e-05_gwas-window1000000_eqtl-window1_snp-gene-pairs-considered.txt", col.names=c("chr", "snp.pos", "gwas-pvalue", "trait", "eqtl-pvalue", "gwas_file", "feature", "eqtl_file"), skip=1, header=FALSE)
...
write.table(all_snp_gene_pairs, file="/users/mgloud/projects/insulin_resistance/eriks_bonus/output/all_snp_gene_pairs_prefiltering.txt", quote=FALSE, col.names=TRUE, row.names=FALSE, sep="\t")
```
...
```{r}
testable_snps = read.table("/users/mgloud/projects/insulin_resistance/eriks_bonus/output/t2d-tg-ldl_gwas-t2d-tg-ldl_ir-tissues_gwas-pval1e-05_eqtl-pval1e-05_gwas-window1000000_eqtl-window1_coloc-tests.txt", header=TRUE)
...
folders = dir("/users/mgloud/projects/brain_gwas/output/eriks-full-analysis-enhanced-for-stefan/")
tabs = list()
err_tab = list()
skip_tab = list()
i = 1
...
{
	files = dir(paste0("/users/mgloud/projects/brain_gwas/output/eriks-full-analysis-enhanced-for-stefan/", folder))
	...
	if (length(error) > 0)
	{
		err_tab[[i]] = read.table(paste("/users/mgloud/projects/brain_gwas/output/eriks-full-analysis-enhanced-for-stefan", folder, error, sep="/"), header=FALSE, sep="\t", fill=TRUE, col.names = c("gwas_file", "eqtl_file", "snp.chrom", "snp.pos", "restrict_gene", "trait", "error", "error2"))
		err_tab[[i]]$error = paste(err_tab[[i]]$error, err_tab[[i]]$error2, sep="|")
	}
	if (length(skip) > 0)
	{
		skip_tab[[i]] = read.table(paste("/users/mgloud/projects/brain_gwas/output/eriks-full-analysis-enhanced-for-stefan", folder, skip, sep="/"), header=FALSE, sep="\t", fill=TRUE, col.names = c("gwas_file", "eqtl_file", "snp.chrom", "snp.pos", "feature", "error", "gwas_data"))
	}

	for (file in files)
	{
		tabs[[i]] = read.table(paste("/users/mgloud/projects/brain_gwas/output/eriks-full-analysis-enhanced-for-stefan", folder, file, sep="/"), header=TRUE)
		i = i + 1
	}
}
...
write.csv(results, file="/users/mgloud/projects/insulin_resistance/eriks_bonus/output/full_coloc_results_qced.txt", quote=FALSE, sep="\t", col.names=TRUE, row.names=FALSE)
```
